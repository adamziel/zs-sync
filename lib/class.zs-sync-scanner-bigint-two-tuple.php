...
class ZS_Sync_Scanner_Bigint_Two_Tuple implements ZS_Sync_Scanner_Table_Type {
	...
	public static function mark_deletions(
		...
		$from_pk_head = $from_pk[0];
		$from_pk_tail = $from_pk[1];
		$to_pk_head = $to_pk[0];
		$to_pk_tail = $to_pk[1];
		
		$sql = <<<SQL
			SELECT * FROM wp_sync_metadata__bigint_two_tuple 
			WHERE hash_value IS NOT NULL 
			AND (primary_key_head, primary_key_tail) >= ($from_pk_head, $from_pk_tail)
			AND (primary_key_head, primary_key_tail) <= ($to_pk_head, $to_pk_tail)
		SQL;
		...
		foreach($rows as $row) {
			if($bloom_filter->exists( $row->hash_value )) {
				$deleted_pks[] = [
					'head' => (int) $row->primary_key_head,
					'tail' => (int) $row->primary_key_tail
				];
			}
		}
		...
		foreach ($deleted_pks as $pk) {
			$conditions[] = "(primary_key_head = {$pk['head']} AND primary_key_tail = {$pk['tail']})";
		}
		
		$sql = <<<SQL
			UPDATE wp_sync_metadata__bigint_two_tuple 
			SET hash_value = NULL 
			WHERE
